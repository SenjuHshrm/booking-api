...
let getMsgRoom = async (id: string): Promise<string[]> => {
  try {
    let rooms: IMessageRoomSchema[] = <IMessageRoomSchema[]>(
      await MessageRoom.find({ members: id }).exec()
    );
...
let saveMsgAndCreateRoom = async (
  res: Response,
  receiverId: string,
  data: IMessageInput
): Promise<Response> => {
  try {
    let newRoom = await new MessageRoom({
...
let getMessageRooms = async (res: Response, id: string): Promise<Response> => {
  try {
    let rooms: IMessageRoomSchema[] = <IMessageRoomSchema[]>(
      await MessageRoom.find({ members: id })
        .populate({ path: "members", select: "_id name img" })
        .exec()
    );
    let resp: Promise<IMessageRoom[]> = Promise.all(
      rooms.map(async (room: IMessageRoomSchema) => {
        let m: IMessageSchema = <IMessageSchema>(
          await Message.findOne({ roomId: room.id })
            .sort({ createdAt: -1 })
            .exec()
        );
        return {
...
let getMessageThread = async (
  ...
  try {
    let msgs: IMessageSchema[] = <IMessageSchema[]>await Message.find({
      roomId,
    })
